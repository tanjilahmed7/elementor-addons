var CountDown = function($scope, $) {
    var $coundDown = $scope.find(".ht-countdown-wrapper").eq(0),
        $countdown_id =
    ...
    jQuery(document).ready(function($) {
        "use strict";
        var countDown = $("#ht-countdown-" + $countdown_id);

        countDown.countdown({
            end: function() {
                if ($expire_type == "text") {
                    countDown.html(
                        '<div class="ht-countdown-finish-message"><h4 class="expiry-title">' +
                            $expiry_title +
                            "</h4>" +
                            '<div class="ht-countdown-finish-text">' +
                            $expiry_text +
                            "</div></div>"
                    );
...
jQuery(window).on("elementor/frontend/init", function() {
    elementorFrontend.hooks.addAction(
        "frontend/element_ready/ht-countdown.default",
        CountDown
    );
});
